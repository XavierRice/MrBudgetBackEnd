...
describe("GET /transactions", () => {
  it("should return the transaction array", async () => {
    ...
    expect(response.body).toEqual(transactionsArr);
  });
});

describe("GET /transactions/:id", () => {
  it("should return the specific transaction by ID", async () => {
    const transactionId = transactionsArr[0].id;
    const response = await request(app).get(`/transactions/${transactionId}`);

    expect(response.status).toBe(200);
    expect(response.body).toEqual(
      transactionsArr.find((transaction) => transaction.id === transactionId)
    );
  });
...
describe("DELETE /transactions/:id", () => {
  it("should delete an existing transaction", async () => {
    ...
    expect(response.status).toBe(200);
    expect(
      transactionsArr.some((transaction) => transaction.id === transactionId)
    ).toBeFalsy();
  });
});
